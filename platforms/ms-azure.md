...
| `manual` trigger | Application-managed `Set Blob Tier`, `Copy Blob`, or `Delete Blob` workflow | Azure lifecycle management has no manual trigger. |
...
| `age` + transition to `Cool` | `tierToCool.daysAfterModificationGreaterThan` | `from` must identify a value guaranteed to equal the blob's last-modified time, and `after` must be an exact non-negative integral number of days. |
...
| Access-driven `Cool` to `Hot` | Azure `enableAutoTierToHotFromCool` | This is a provider-specific optimization and is not a general mapping of the generic `manual` trigger. |
